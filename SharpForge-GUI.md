...
--- Transpiler ---
Enabled                     : [x] // when checked, Run preview includes sf-transpile
...
--- Builder ---
Enabled                     : [x] // when checked, Run preview includes sf-build
...
【Run】 // executes the command lines currently in the textarea as a `cmd.exe /D /C` chain joined with `&&`
【Replay】
...
Run shows the command lines it will execute. Manual edits are used by Run until another field change regenerates the preview, and execution joins the non-empty lines with `&&` so later commands run only when earlier commands succeed.
Generated Warcraft launch commands are executed as detached processes after preceding `&&`-chained commands succeed, avoiding `cmd.exe start` quoting issues.
When Builder output is a real `.w3x` file, the generated Warcraft launch command uses the copied build map (`demo.sf-build.w3x`) rather than the source map.
